...
export function useTasks(): UseTasksState {
  ...
  // initial load: localstorage -> public JSON -> fallback generated dummy
  useEffect(() => {
    let isMounted = true;
    async function load() {
      ...
      try {
        const local = localStorage.getItem("taskglitch-tasks");
        if (local) {
  ...
  useEffect(() => {
    if (!loading && fetchedRef.current) {
      localStorage.setItem("taskglitch-tasks", JSON.stringify(tasks));
    }
  }, [tasks, loading]);
